\documentclass[12pt]{article}
\input{settings/paquetes}
\input{settings/comandos}
\input{settings/enpie}
\input{settings/entornos}
\begin{document}
\input{contenido/titulo}
\tableofcontents
\newpage
\listoftables
\newpage
\listoffigures
\newpage
	La encuesta se realizó en la comunidad campesina de \comunidad\- del distrito de distrito, provincia de provincia departamento del Cusco.\\
	\\
	Para esto se considero informacion general de la población, tomando en cuenta el genero, la edad y el recurso que utilizan, en este caso si usan qocha.\\
	\\
	Del genero de las personas encuestadas se puede mostrar lo siguiente:
	\begin{figure}[H]
	\centering
	<<uno, echo = FALSE, fig.width=4, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	graph1 <- ggplot(datos2, aes(Sexo)) +
  geom_bar( fill = rgb(0.36, 0.79, 0.95), colour = "black") + xlab("") + ylab("") 
	graph1 + theme_bw()
	@
	\caption{Sexo de los encuestados de \comunidad}
	\end{figure}
	Del gráfico se evidencia que el genero masculino es mayor. El alto porcentaje del sexo masculino sugiere que son numerosos en la comunidad de \comunidad en comparación con las mujeres. Esto podría tener varias implicaciones, como patrones de migración de género, roles tradicionales en la sociedad. Sin embargo el porcentaje de las mujeres indican que son una minoría en la sociedad. Finalmente estos datos puden reflejar dinámicas sociales, económicas y culturales particulares en la comunidad de Chilloroya
	\section{Datos generales}
	Edad de los encuestados, de la encuesta realizada se tiene la edad de los participantes, como se muestra a continuacion "mejorar, no redundancia":
	\begin{figure}[H]
	\centering
	<<dos, echo = FALSE, fig.width=5, fig.height=6>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#con una sola barra, quedarse con este
  p <- ggplot(datos2, aes(edad, fill = Sexo)) +
  geom_bar() +
    coord_flip()
  p + theme_bw()
  
@
             	
	\caption{Edad de los encuestados de \comunidad}
	\end{figure}
	
Se puede observar que el rango de edades va desde 23 hasta 71 años, siendo las edades de mayor frecuencia el 48 y 58 años los cuales pertenenen al sexo masculino, el alto porcentaje sugiere que la poblacion en estas edades es representativa, esto podría indicar que la comunidad tiene una cantidad significativa de personas adultas lo cual está involucrado en roles de liderazgo, trabajo y toma de decisiones en la \comunidad; asimismo, existen personas entrevistadas de sexo femenino donde la frecuencia es de 43 y 55 años, asimismo se manifiestan en la misma proporción, esto podría sugerir que existen mujeres que tienen capacidad de tomar decisiones en la \comunidad, además existen mujeres Jóvenes de 28 y 31 años en menor cantidad en esta \comunidad, talvez existen factores de migración a otras áreas en busca de educacion o empleo.
	\section{Informacion sobre la vivienda}
Se pregunto respecto a la tenencia de la propiedad en la que habitan los encuestados, de la cual se oberva que el 100\% es propia, como se puede observar en la grafica:
	\begin{figure}[H]
	\centering
	<<tres, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	graph3 <- ggplot(datos2, aes(tenencia)) +
  geom_bar( fill = rgb(0.36, 0.79, 0.95), colour = "white") + xlab("propiedad") + ylab("frecuencia")  
	graph3 + theme_bw()
	@
	\caption{Tenencia de la propiedad de los pobladores de \comunidad}
	\end{figure}
	Como se puede observar en el gráfico se tiene que los pobladores de la comunidad de \comunidad tienen casa propia lo que podria indicar una cierta estabilidad en términos de vivienda y posesión de propiedades en la zona. Esto obedece a que las construciones se encuentra inmersa en ecosistemas naturales que conserva, transforma o deteriora.\\
	\\
	Se consulto el tiempo, años, que viven los pobladores en la comunidad, de lo cual se tiene como resultado lo siguiente:
	\begin{figure}[H]
	\centering
	<<cuatro, echo = FALSE, fig.width=5, fig.height=6>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$añosh <- as.numeric(datos2$añosh)
	
	graph4 <- ggplot(datos2, aes(añosh)) +
  geom_bar( fill = rgb(0.36, 0.79, 0.95), colour = "black") + xlab("Años") + ylab("Cantidad") + coord_flip()  
	graph4 + theme_bw()
	@
	\caption{Años de permanencia de los pobladores en \comunidad}
	\end{figure}
	Se observa que los años que habitan los pobladores van desde 20 a 71, los años que tienen mas frecuencia son de 48 y 58, esto obedece a que los aspectos culturales y ancestrales predominan en la comunidad. Asimismo; estos pobladores son jefes de familia que han heredado costumbres. Finalmente hay personas que habiatan entre 20 a 44 años en la comunidad \\
	\\
	Del idioma que los encuestados se identifico que la todos habla español y quechua, como se grafica:
	\begin{figure}[H]
	\centering
	<<cinco, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	graph5 <- ggplot(datos2, aes(Idioma)) +
  geom_bar( fill = rgb(0.36, 0.79, 0.95), colour = "black") + xlab("Cantidad") + ylab("Idioma")  
	graph5 + theme_bw()
	@
	\caption{Idioma que habla el poblador}
	\end{figure}
	Se observa que el total de los encuestados hablan español y quechua. Esto explica que los pobladores de la comunidad de \comunidad son bilingües, esto sugiere que la educación en la comunidad marcó un hito importante en la lengua, donde se intercambió quechua fortalece nuestra identidad nacional.
	\section{Informacion de servicios}
	De la fuente de agua que los pobladores se abastecen se identifico lo siguiente:
	\begin{figure}[H]
	\centering
	<<seis, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	graph6 <- ggplot(datos2, aes(fuentea)) +
  geom_bar( fill = rgb(0.36, 0.79, 0.95), colour = "black") + xlab("Fuente de agua") + ylab("Cantidad")  
	graph6 + theme_bw()
	@
	\caption{Fuente de agua para consumo de agua de los pobladores de \comunidad}
	\end{figure}
	La fuente de manantial es la mas usada por los pobladores de la comunida de \comunidad las cuales tienen un origen subterraneo que emergen espontanemente en la superfcie. Seguido del abastecimeinto de pileta pública donde son controlados por los directivos de las juntas administradoras de servicios de saneamiento (JASS), esto quiere decir que la población de la mencionado comunidad realizan actividades de limpieza y desinfección, en consecuencia tambien consumen agua proveniente de rio o laguna, finalmente hay un pequeño grupo que se abastece de agua de las acequias.\\
	\\
	La eliminacion de basura en las viviendas de los pobladores es la siguiente
	\begin{figure}[H]
	\centering
	<<siete, echo = FALSE, fig.width=8, fig.height=5>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$basura[datos2$basura == "Hoyos"] <- "Hoyo"
	
	graph7 <- ggplot(datos2, aes(basura)) +
  geom_bar( fill = rgb(0.36, 0.79, 0.95), colour = "black") + xlab("Tipo de eliminacion") + ylab("Cantidad")  + coord_flip() 
	graph7 + theme_bw() 
	 
	@
	\caption{Eliminacion de la basura en \comunidad}
	\end{figure}
	Los tipos de eliminacion con mayor frecuencia es el botadero. Esto sugiere que existe una organización de los pobladores y de las instituciones que intervinene en el cuidado del medio ambiente, además se precisa que existen pobladores que entierran los desechos para propiciar la descomposición de la basura, tambien se observa que hay comuneros que realizan el hoyo para almacenar la basura, un pequeño grupo realiza la actividad del quemado, finalmente un porcentaje minoritario usa el relleno sanitario de la comunidad.\\
	\\
	Para la preparacion de sus alimentos las personas utilizan los siguientes combustibles:
	\begin{figure}[H]
	\centering
	<<ocho, echo = FALSE, fig.width=5, fig.height=7>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#table(datos2$combustible)
	#reemplazando los valores vacios en NA
	datos2$combustible[datos2$combustible == ""] <- NA
	
	ggplot(datos2, aes(combustible)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	@
	\caption{Uso de combustible para preparacion de alimentos en \comunidad}
	\end{figure}
	Los pobladores usan con mas frecuencia bosta y bosta y gas, los habitantes aprovechan el estiercol del ganado para preparar alimentos en lugares donde es muy complicado conseguir leña de madera asimismo el Gas es utilizado por la familias en la comunidad de \comunidad, esto sugiere que el poder de adquisicion de este elemento se ha incrementado en el consumo, dado que se les hace mas accesible con respecto al precio, el gobierno destina un descuento exclusivo mediante fondo de inclusion Social Energético (FISE) esto se llama compensacion social y acceso a GLP. Existe una pequeña proporcion de familias que usan gas y leña al mismo tiempo. Cabe mencionar que la población usa el Gas cuando el bosta y leña no están en cponsiciones adecuadas o están en temporadas de iniverno la cual se les hace muy complicado preprarar los alimentos.\\
	\\
	Las enfermedades que mas aquejan a la poblacion son las siguientes:
	\begin{figure}[H]
	\centering
	<<nueve, echo = FALSE, fig.width=5, fig.height=5>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#reemplazando los valores vacios en NA
	datos2$enfermedad[datos2$enfermedad == ""] <- NA
	datos2$enfermedad[datos2$enfermedad == "Gripe"] <- "Tos"
	datos2$enfermedad[datos2$enfermedad == "Diarreicas"] <- "Diarreicas, Infecciones"
	datos2$enfermedad[datos2$enfermedad == "Infecciones"] <- "Diarreicas, Infecciones"
	
	ggplot(datos2, aes(enfermedad)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95)) + 
	  coord_flip()
	
	@
	\caption{Enfermedades que aquejan a los pobladores de \comunidad}
	\end{figure}
	Se observa en el gráfico que las enfermedades que mas aqueja a las familias de la comunidad de \comunidad son las enfermedades diarreicas e infecciones, esto sugiere que la falta de higiene, la ingestión del agua y alimentos contaminados son las vías por medio de las cuales se adquieren estos malestares. Seguido de un porcentaje predominante señalan que no tienen ningna enfermedad.\\
	\\
	Además se oberva que existen familias que tienen tos, esto afirma que la altitud del lugar influye en la salud de las personas, además la causa mas frecuente respecto a este mal es la enfermedad pulmonar obstructiva, generalmente este caso afecta a los niños y personas que se encuentran en la tercera edad.Finalmente se tiene las enfermedades que mas afecta a la población son la Infecciones a la piel, infecciones y problemas respiratorios en la misma proporción.\\
	\\
	Las personas que sufren de alguna enfermedad se atienden en los siguientes lugares:
	\begin{figure}[H]
	\centering
	<<once, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#reemplazando los valores vacios en NA
	datos2$atiendef[datos2$atiendef == ""] <- NA
	datos2$atiendef[datos2$atiendef == "SIS"] <- "Posta de salud"
	
	datos2 <- datos2[!is.na(datos2$atiendef), ]
	ggplot(datos2, aes(atiendef)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	

	@
	\caption{Lugar de atencion cuando se enferman}
	\end{figure}
  Los datos de la gráfica muestran que los pobladores de la comunidad de \comunidad acuden a la posta de salud mas cercana, esto indica que estos establecimientos de salud son actores esenciales que atienden emergencias o contingencias en el lugar. Seguido de un pequeño grupo de familias que acuden a las farmacias  de manera periódica en la capital de distrito, además hay pobladores que remedian sus enfermedades en su vivienda con tratamientos caseros.\\
  \\
	Se consulto si estas personas cuentan con seguro medico, de lo cual se obtuvo el siguiente resultado:
	\begin{figure}[H]
	\centering
	<<doce, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#reemplazando los valores vacios en NA
	datos2$seguro[datos2$seguro == ""] <- NA
	datos2$seguro[datos2$seguro == "SIS"] <- "si"
	
	datos2 <- datos2[!is.na(datos2$seguro), ]
	ggplot(datos2, aes(seguro)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	
	@
	\caption{Uso de un seguro medico}
	\end{figure}
	Los resultados de este gráfico indican que sí cuentan con seguro médico, esto podría sugerir que la población desde su nacimiento tiene derecho a la salud o al Seguro Integral de Salud(SIS), asimismo existen personas que cuentan con seguros ya sea privado o Essalud. Seguido de un pequeño porcentaje de pobladores que no tienen seguro de salud, esto indica que perdieron el SIS por que han ingresado a otro seguro de manera temporal o la misma población migra a otras comunidades o ciudades lo cual dificulta que puedan acceder a un seguro.\\
	\\
	Se consulto respecto a la preferencia de emisoras radiales que se escucha, al respecto se tiene como primera emisora de preferencia la siguiente:
	\begin{figure}[H]
	\centering
	<<one, echo = FALSE, fig.width=8, fig.height=5>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$Emi1[datos2$Emi1 == ""] <- NA
	
	datos2$Emi1[datos2$Emi1 == "No"] <- "Ninguno"
	datos2$Emi1[datos2$Emi1 == "Emisora"] <- "Velille"
	datos2$Emi1[datos2$Emi1 == "Victoria"] <- "Velille"
	
	ggplot(datos2, aes(Emi1)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95)) + 
	  coord_flip()
	
	
	@
	\caption{Primera emisora que se escucha}
	\end{figure}
	Como se puede observar la emisora con mayor audiencia es Velille, seguido de emisora Choquevelille, ademá existe un porcentaje de la población que no tiene preferencia radial. Un grupo de familias Sintoniza Radio Livitaca, seguido de un grupo limitado escuchan radio Melodía Arequipa, Exitosa y Chumbivilcas respectimanete.\\
	\\
	El horario que los encuestados prefieren escuchar es el siguiente:
	\begin{figure}[H]
	\centering
	<<two, echo = FALSE, fig.width=7, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$emisora.1.horario[datos2$emisora.1.horario == ""] <- NA
	
	datos2$emisora.1.horario[datos2$emisora.1.horario == "05:08:00 a. m."] <- "05:00:00 a. m."
	datos2$emisora.1.horario[datos2$emisora.1.horario == "07:09:00 a. m."] <- "07:00:00 a. m."
	datos2$emisora.1.horario[datos2$emisora.1.horario == "07:10:00 a. m."] <- "07:00:00 a. m."
	#table(datos2$emisora.1.horario)
	#probando eliminando NA
	datos2$emisora.1.horario <- substr(datos2$emisora.1.horario, 1, nchar(datos2$emisora.1.horario)- 9)
	
	datos2 <- datos2[!is.na(datos2$emisora.1.horario), ]
	ggplot(datos2, aes(emisora.1.horario)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Horario de emisora preferida}
	\end{figure}
	Los datos de la gráfica señalan que el horario habitual que sintonizan la radio en la \comunidad es a las 7:00 a.m. Además existen familias que encienden la radio a las 5:00 a.m. y 6:00 de la mañana, tambien hay un horario de 8:00 y 12:00 del medio día que prefieren escuchar la radio. Finalmente un grupo reducido de personas sintonizan a las 4:00 de la madrugada.\\
	\\
Emisoras y horarios preferidos:
	\begin{figure}[H]
	\centering
	<<three, echo = FALSE, fig.width=7, fig.height=5>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	library(ggplot2)
  
  datos2$Emi1[datos2$Emi1 == ""] <- NA
	
	datos2$Emi1[datos2$Emi1 == "No"] <- "Ninguno"
	datos2$Emi1[datos2$Emi1 == "Emisora"] <- "Velille"
	datos2$Emi1[datos2$Emi1 == "Victoria"] <- "Velille"
  
	
	datos2$emisora.1.horario[datos2$emisora.1.horario == "05:08:00 a. m."] <- "05:00:00 a. m."
	datos2$emisora.1.horario[datos2$emisora.1.horario == "07:09:00 a. m."] <- "07:00:00 a. m."
	datos2$emisora.1.horario[datos2$emisora.1.horario == "07:10:00 a. m."] <- "07:00:00 a. m."
  datos2$emisora.1.horario <- substr(datos2$emisora.1.horario, 1, nchar(datos2$emisora.1.horario)- 9)
	
	
	datos2 <- datos2[!is.na(datos2$Emi1), !is.na(datos2$emisora.1.horario), ]
	
	ggplot(datos2, aes(Emi1, emisora.1.horario)) + 
	  geom_count(colour = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Emisoras y horarios preferidos}
	\end{figure}
	Como se muestra en el gráfico los horarios preferidos  que sintonizan en la comunidad de \comunidad es a las 7:00 a.m. lo cual es escuchada por 2 personas, hay solo una persona que enciende la radio en los horarios de 4:00, 6:00, 8:00 y 12:00 del meridiano.\\ 
	\\
	
	Se consulto respecto a la preferencia de emisoras radiales que se escucha, al respecto se tiene como primera emisora de preferencia la siguiente:
	\begin{figure}[H]
	\centering
	<<four, echo = FALSE, fig.width=8, fig.height=5>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$Emi2[datos2$Emi2 == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$Emi2), ]
	ggplot(datos2, aes(Emi2)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95)) + 
	  coord_flip()
	@
	\caption{Seguna emisora que se escucha}
	\end{figure}
	De la primera emisora que mas se escucha en la comunidad de \comunidad se identificó que la radio Laramani y Altiva Canas son de mayor preferencia por los pobladores de la zona; sin embargo las emisoras radiales Canas, Espinar, Livitaca, RPP y Velille tienen una menor preferencia.\\
	\\
	El horario que los encuestados prefieren escuchar es el siguiente:
	\begin{figure}[H]
	\centering
	<<five, echo = FALSE, fig.width=8, fig.height=5>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$emisora.2.horario[datos2$emisora.2.horario == ""] <- NA
	datos2$emisora.2.horario[datos2$emisora.2.horario == "05:08:00 a. m."] <- "05:00:00 a. m."
	datos2$emisora.2.horario[datos2$emisora.2.horario == "07:10:00 a. m."] <- "07:00:00 a. m."
	datos2$emisora.2.horario[datos2$emisora.2.horario == "05:00:00 p. m."] <- "17:00:00 p. m."
	datos2$emisora.2.horario[datos2$emisora.2.horario == "07:00:00 p. m."] <- "19:00:00 p. m."
	
	
	
	
	#probando eliminando NA
	datos2$emisora.2.horario <- substr(datos2$emisora.2.horario, 1, nchar(datos2$emisora.2.horario)- 9)
	
	datos2 <- datos2[!is.na(datos2$emisora.2.horario), ]
	ggplot(datos2, aes(emisora.2.horario)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	
	@
	\caption{Horario de emisora preferida}
	\end{figure}
	Los datos de la gráfica muestran que los encuestados tienen una preferencia de escuchar la radio como como segunda opción en el horario de 5:00 y 7:00 de la mañana; finalmente, e oberva que exiten familias que prefieren escuchar en los horarios de 6:00, 8:00 de la mañana y 17:00 y 19:00 de la noche proporcionalmente.\\
	\\
	Emisora y horarios preferidos: 
	\begin{figure}[H]
	\centering
	<<six, echo = FALSE, fig.width=8, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$Emi2[datos2$Emi2 == ""] <- NA
	
	#datos2$emisora.2.horario[datos2$emisora.2.horario == ""] <- "no escucha"
	datos2$emisora.2.horario[datos2$emisora.2.horario == "05:08:00 a. m."] <- "05:00:00 a. m."
	datos2$emisora.2.horario[datos2$emisora.2.horario == "07:10:00 a. m."] <- "07:00:00 a. m."
	datos2$emisora.2.horario[datos2$emisora.2.horario == "05:00:00 p. m."] <- "17:00:00 p. m."
	datos2$emisora.2.horario[datos2$emisora.2.horario == "07:00:00 p. m."] <- "19:00:00 p. m."
	
	datos2$emisora.2.horario <- substr(datos2$emisora.2.horario, 1, nchar(datos2$emisora.2.horario)- 9)
	
	datos2 <- datos2[!is.na(datos2$Emi2), !is.na(datos2$emisora.2.horario), ]
	
	ggplot(datos2, aes(Emi2, emisora.2.horario)) +
	  geom_count(colour = rgb(0.36, 0.79, 0.95))
	View(datos2)
	@
	\caption{Emisoras y horarios preferidos}
	\end{figure}
	Se observa tambien en este gráfico que el horario preferido para escuchar la segunda emisora es a las 7:00 y 5:00 de la mañana; asimismo, hay pobladores que sintonizan en el horario de 6:00 y 8:00 a.m. finalmente hay familias que encienden la radio en el horario de la noche las cuales son 17:00 p.m.  19:00 p.m.
	
	
	\section{Datos agropecuarios}
	De la actividad agropecuaria, se pregunto que cultivos como primera opcion realizan, de lo cual se obtuvo la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<seven, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	datos2$cultivo1[datos2$cultivo1 == ""] <- NA
	datos2$cultivo1[datos2$cultivo1 == "Ninguno"] <- NA
	datos2$cultivo1[datos2$cultivo1 == "Papa"] <- "papa"
	datos2$cultivo1[datos2$cultivo1 == "No"] <- "Ninguno"
	
	
	#datos2$cultivo1[datos2$cultivo1 == "avena"] <- "Avena"
	
	datos2 <- datos2[!is.na(datos2$cultivo1), ]
	ggplot(datos2, aes(cultivo1)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))

	#table(datos2$cultivo1)
	
	@
	\caption{Cultivo al que se dedican las personas}
	\end{figure}
	En la comunidad de \comunidad se tiene que el producto que mas se cultiva es la papa en gran proporción, esto sugiere que este tubérculo se ha convertido en un notable impulsor de la economía de la comunidad tal como señalan, asu ves dota de mayores ingresos a las familias, además un pequeño porcentaje cultiva papa amarga que generalmente lo extraen de las partes altas del lugar. Tambien se puede observar que cultivan Avena en menor proporción, lo cual podría indicar que es de autoconsumo o simplemente sirve para alimentar a los animales de casa, finalmete se pudo ver que hay familias que no se dedican a la actividad agrícola.\\
	\\
	De la cantidad de cultivo se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<eight, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$qcultivo1[datos2$qcultivo1 == ""] <- NA
	datos2$qcultivo1[datos2$qcultivo1 == "Tres surcos"] <- NA
	datos2$cultivo1[datos2$cultivo1 == ""] <- NA
	datos2$cultivo1[datos2$cultivo1 == "Ninguno"] <- NA
	datos2$cultivo1[datos2$cultivo1 == "Papa"] <- "papa"
	datos2$cultivo1[datos2$cultivo1 == "No"] <- "Ninguno"
	
	#table(datos2$qcultivo1)
	datos2$qcultivo1 <- as.numeric(datos2$qcultivo1)
	datos2 <- datos2[!is.na(datos2$qcultivo1), ]
	
	ggplot(datos2, aes(qcultivo1)) +
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Primera opcion de cultivo en \comunidad \- en cantidades}
	\end{figure}
	
	De los datos del gráfico se puede observar que de las personas encuestadas cultivan papa  en una cantidad de 4000 kilos, seguido de la papa amarga donde 2 personas cultivan 2500 kilos, asu ves una persona  encuestada cultiva 1500 Kg de avena.\\
	\\
	Cantidad de cultivo por producto:
	\begin{figure}[H]
	\centering
	<<oneh, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	datos2$qcultivo1[datos2$qcultivo1 == ""] <- NA
	datos2$qcultivo1[datos2$qcultivo1 == "Tres surcos"] <- NA
	
	datos2$cultivo1[datos2$cultivo1 == ""] <- NA
	datos2$cultivo1[datos2$cultivo1 == "Papa"] <- "papa"
	datos2$cultivo1[datos2$cultivo1 == "No"] <- "Ninguno"
	
	datos2$qcultivo1 <- as.numeric(datos2$qcultivo1)
	
	#datos2<- datos2[!is.na(datos2$cultivo1), !is.na(datos2$qcultivo1), ]
	
	datos2 <- datos2[!is.na(datos2$cultivo1), ]
	datos2 <- datos2[!is.na(datos2$qcultivo1), ]
	
	ggplot(datos2, aes(cultivo1, qcultivo1)) + 
	  geom_count(colour = rgb(0.36, 0.79, 0.95)) +
	  scale_size_area()
	
	#View(datos2)
	
	@
	\caption{Cantidad de cultivo por producto}
	\end{figure}
	El gráfico muestra la cantidad de cultivo por producto, dode se aprecia que la producción de la papa es de 40000 kilos en la comunidad de \comunidad, seguido de la papa amarga lo cual se produce en 2500 kilos y finalmente la avena que tiene una producción 1971 kilos.\\
	\\
	Método de cultivo de primera opción:
	\begin{figure}[H]
	\centering
	<<twelve, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#viendo los datos de la variable mcultivo2
	#table(datos2$mcultivo2)

	#reemplazando los valores vacios en NA
	datos2$mcultivo1[datos2$mcultivo1 == ""] <- NA
	
	#probando eliminando NA
	datos2 <- datos2[!is.na(datos2$mcultivo1), ]
	ggplot(datos2, aes(mcultivo1)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	@
	\caption{Metodo de cultivo de la primera opcion de cultivo}
	\end{figure}
	El dato del gráfico muestra que el secano es el primer método como opción en la \comunidad, cabe señalar que existe el agua de la lluvia donde propicia la producción natural.\\
	\\
	De la actividad agropecuaria, se pregunto que cultivos como segunda opcion realizan, de lo cual se obtuvo la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<nine, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	datos2$cultivo2[datos2$cultivo2 == ""] <- NA
	datos2$cultivo2[datos2$cultivo2 == "no"] <- NA
	datos2$cultivo2[datos2$cultivo2 == "ninguno"] <- "no"
	
	
	datos2 <- datos2[!is.na(datos2$cultivo2), ]
	ggplot(datos2, aes(cultivo2)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	

	
	
	@
	\caption{Cultivo al que se dedican las personas como segunda opcion}
	\end{figure}
	Se observa que el cultivo de avena es la segunda opción mas rentable en la produccion por parte de las personas en la \comunidad.\\
	\\
	De la cantidad de cultivo se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<ten, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$qcultivo2[datos2$qcultivo2 == ""] <- NA
	datos2$qcultivo2[datos2$qcultivo2 == "En proyecto"] <- NA
	datos2$cultivo2[datos2$cultivo2 == ""] <- "Ninguna"
	datos2$cultivo2[datos2$cultivo2 == "no"] <- NA
	datos2$cultivo2[datos2$cultivo2 == "ninguno"] <- NA
	
	datos2$qcultivo2 <- as.numeric(datos2$qcultivo2)
	datos2 <- datos2[!is.na(datos2$qcultivo2), ]
	datos2 <- datos2[!is.na(datos2$cultivo2), ]
	
	ggplot(datos2, aes(cultivo2, qcultivo2), na.rm = TRUE) +
	  geom_count(colour = rgb(0.36, 0.79, 0.95)) +
	  scale_size_area()
	  
	
	
	@
	\caption{Cantidad de cultivo de la segunda opcion de cultivo en \comunidad}
	\end{figure}
	El dato del gráfico afirma que la produccion de la avena en la comunidad de Chilloroya asciende a los 10000 kilos. como segunda opción de cultivo\\
	\\
	Método de cultivo de la segunda opción:
	\begin{figure}[H]
	\centering
	<<eleven, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#viendo los datos de la variable mcultivo2
	#table(datos2$mcultivo2)

	#reemplazando los valores vacios en NA
	datos2$mcultivo2[datos2$mcultivo2 == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$mcultivo2), ]
	ggplot(datos2, aes(mcultivo2)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Metodo de cultivo de la segunda opcion de cultivo en \comunidad}
	\end{figure}
	El dato del gráfico indica que el método de cultivo de la segunda opción(avena)en la \comunidad es el secano, sugiere que la población usa el agua de la lluvia mas no de otras fuentes de abastecimientos de agua para la producción.\\
	\\
	
	De la actividad agropecuaria, se pregunto que cultivos como tercera opcion realizan, de lo cual se obtuvo la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<thirteen, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#
	#table(datos2$cultivo3)
	
	datos2$cultivo3[datos2$cultivo3 == ""] <- NA
	datos2 <- datos2[!is.na(datos2$cultivo3), ]
	ggplot(datos2, aes(cultivo3)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	@
	\caption{Cultivo al que se dedican las personas como tercera opcion}
	\end{figure}
	Se observa que el cultivo que a la que las personas se dedican como tercera opción es Rye grass. Lo cual podría cocluir que este producto sirve para alimentar a las ovejas, caballos y terneras de recría en la comunidad, tambien podría indiacar que es una energía metabolizable para los animales\\
	\\
	De la cantidad de cultivo se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<fourteen, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#table(datos2$qcultivo3)
	datos2$cultivo3[datos2$cultivo3 == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$cultivo3), ]
	datos2 <- datos2[!is.na(datos2$qcultivo3), ]

	
	ggplot(datos2, aes(cultivo3, qcultivo3), na.rm = TRUE) +
	  geom_count(colour = rgb(0.36, 0.79, 0.95))
	@
	\caption{Cantidad de cultivo de la tercera opcion de cultivo en \comunidad}
	\end{figure}
	Se evidencia que la cantidad  de produccion de cultivo como tercera opción es la Rye Grass donde llega a los 5000 kilos en la comunidad de Chilloroya.\\
	\\
	
	
	De la actividad agropecuaria, se pregunto que animales cria segunda opcion, de lo cual se obtuvo la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<fifteen, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$animal1[datos2$animal1 == ""] <- NA
	datos2$animal1[datos2$animal1 == "Ninguno"] <- NA
	datos2$animal1[datos2$animal1 == "70"] <- NA
	
	datos2$animal1[datos2$animal1 == "alpaca"] <- "Alpaca"
	datos2$animal1[datos2$animal1 == "alpacas"] <- "Alpaca"
	
	
	datos2 <- datos2[!is.na(datos2$animal1), ]
	ggplot(datos2, aes(animal1)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))

	
	
	@
	\caption{Animales al que se dedican las personas como primera opcion}
	\end{figure}
	Del gráfico se concluye que la crianza a la que las personas se dedican como primera opción es la Oveja y Vaca. Esto podría indicar que la economía de la comunidad de Chilloroya depende en gran medida de estos ovinos; asimismo podrían aprovechar la lana, Carne, leche, cuero y estircol.\\
	\\
	De la cantidad de animales que crian se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<sixteen, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$animal1[datos2$animal1 == ""] <- NA
	datos2$animal1[datos2$animal1 == "alpaca"] <- "Alpaca"
	datos2$animal1[datos2$animal1 == "alpacas"] <- "Alpaca"
	#table(datos2$cabezas1)
	datos2$cabezas1[datos2$cabezas1 == ""] <- NA
	
	#convertir una columna de un df de character a numeric 
	
	datos2$cabezas1 <- as.numeric(datos2$cabezas1)
	datos2 <- datos2[!is.na(datos2$animal1),]
	datos2 <- datos2[!is.na(datos2$cabezas1),]
	
	ggplot(datos2, aes(animal1, cabezas1)) +
	  geom_count(color = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Cantidad de animales criados de primera opcion en funcion a la cantidad y especie en \comunidad}
	\end{figure}
	Los datos del gráfico señalan que en promedio las familias tienen en su poder 100 ovejas, asimismo tienen vacas que se comportan en una mínima cantidad de 22 cabezas.\\
	\\
	Método de crianza de la primera opción: 
	\begin{figure}[H]
	\centering
	<<seventeen, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#viendo los datos de la variable mcultivo2
	#table(datos2$mcultivo2)

	#reemplazando los valores vacios en NA
	datos2$tipo.1[datos2$tipo.1 == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$tipo.1), ]
	ggplot(datos2, aes(tipo.1)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))

	
	@
	\caption{Metodo de crianza de la primera opcion de crianza de animales en \comunidad}
	\end{figure}
	Los datos de la gráfica indican que el método de crianza de primera opción es el pastoreo, seguidodel método del semi estabulado y finalmente el extensivo\\
	\\
	
	De la actividad agropecuaria, se pregunto que animales cria como segunda opción, de lo cual se obtuvo la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<eighteen, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#table(datos2$animal2)
	#si hay datos vacios se reemplazara con NA
	datos2$animal2[datos2$animal2 == ""] <- NA
	datos2$animal2[datos2$animal2 == "Vaca"] <- "Vacuno"
	
	#datos2$animal2[datos2$animal2 == "oveja"] <- "Oveja"
	datos2 <- datos2[!is.na(datos2$animal2), ]
	
	ggplot(datos2, aes(animal2)) +
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	@
	\caption{Animales al que se dedican las personas como segunda opcion}
	\end{figure}
	Los datos del gráfico muestran que las familias crian como segunda opción en gran proporción a la oveja , lo cual supera a la crianza del vacuno y Llama.\\
	\\
	De la cantidad de animales que crian se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<nineteen, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$animal2[datos2$animal2 == ""] <- NA
	datos2$animal2[datos2$animal2 == "Vaca"] <- "Vacuno"
	datos2$animal2[datos2$animal2 == "oveja"] <- "Oveja"
	datos2$cabezas2 <- as.numeric(datos2$cabezas2)
	datos2$cabezas2[datos2$cabezas2 == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$cabezas2), ]
	datos2 <- datos2[!is.na(datos2$animal2), ]
	ggplot(datos2, aes(animal2, cabezas2)) +
	  geom_count(color = rgb(0.36, 0.79, 0.95)) +
	  scale_size_area()
	
	@
	\caption{Cantidad de animales criados de segunda opcion en funcion a la cantidad y especie en \comunidad}
	\end{figure}
	Del gráfico se puede concluir que existen 50 cabezas entre ovejas, vacunos y llamas en crianza como segunda opción\\
	\\ 
	Método de crianza de la segunda opoción:
	\begin{figure}[H]
	\centering
	<<twenty, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#viendo los datos de la variable mcultivo2
	#table(datos2$mcultivo2)

	#reemplazando los valores vacios en NA
	#table(datos2$tipo.2)
	datos2$tipo.2[datos2$tipo.2 == ""] <- NA
	
	#probando eliminando NA
	datos2 <- datos2[!is.na(datos2$tipo.2), ] 
	ggplot(datos2, aes(tipo.2)) + 
	  geom_bar( fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Metodo de crianza de la segunda opcion de crianza de animales en \comunidad}
	\end{figure}
	De acuerdo a los datos del gráfico se oberva que el método de crianza de la segun opción es el pastoreo en la \comunidad, seguido del métedo extensivo.\\
	\\
	
	De la actividad agropecuaria, se pregunto que animales cria como tercera opcion, de lo cual se obtuvo la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<twenty_one, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#table(datos2$animal3)
	#si hay datos vacios se reemplazara con NA
	datos2$animal3[datos2$animal3 == ""] <- NA
	datos2$animal3[datos2$animal3 == "oveja"] <- "Oveja"
	datos2$animal3[datos2$animal3 == "llama"] <- "Llama"
	
	datos2 <- datos2[!is.na(datos2$animal3), ]
	ggplot(datos2, aes(animal3)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Animales al que se dedican las personas como tercera opcion}
	\end{figure}
	Se observa que la crianza a la que las personas se dedican como tercera opción es alpaca y Caballos en gran medida, seguida de Gallinas y Llama que se comportan en menor proporción.\\
	\\
	De la cantidad de animales que crian se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<twenty_two, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	#datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$animal3[datos2$animal3 == ""] <- NA
	datos2$animal3[datos2$animal3 == "oveja"] <- "Oveja"
	datos2$animal3[datos2$animal3 == "llama"] <- "Llama"
	datos2$cabezas3 <- as.numeric(datos2$cabezas3)
	datos2$cabezas3[datos2$cabezas3 == ""] <- NA
	#class(datos2$cabezas3)
	datos2 <- datos2[!is.na(datos2$animal3), ]
	datos2 <- datos2[!is.na(datos2$cabezas3), ]
	
	ggplot(datos2, aes(animal3, cabezas3), na.rm = TRUE) +
	  geom_count(color = rgb(0.36, 0.79, 0.95)) + 
	  scale_size_area()
	
	@
	\caption{Cantidad de animales criados de tercera opcion en funcion a la cantidad y especie en \comunidad}
	\end{figure}
	Del gráfico se concluye que existen 100 llamas que crian como tercera opción en la comunidad de Chilloroya, seguido 30 alpacas, 15 gallinas y 10 caballos que tienen en su poder.\\
	\\
	Del metodo de crianza de esta opcion de animales se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<twenty_three, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#viendo los datos de la variable mcultivo2
	#table(datos2$mcultivo2)

	#reemplazando los valores vacios en NA
	#table(datos2$tipo.3)
	datos2$tipo.3[datos2$tipo.3 == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$tipo.3), ]
	ggplot(datos2, aes(tipo.3)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Metodo de crianza de la tercera opcion de crianza de animales en \comunidad}
	\end{figure}
	El método de crianza que tienen los pobladres a los animales de tercera opción, pricipalmente es el pastoreo en gran medida y el método extensivo en menor proporción.\\
	
	
	\section{Recurso hidrico}
	De las personas que acceden al servicio de las qochas se identifico que solo esta cantidad accede al servicio:
	\begin{figure}[H]
	\centering
	<<trece, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#datos2$accrecurso[datos2$accrecurso == "cada cuantos meses"] <- "algunos meses"
	#datos2$accrecurso[datos2$accrecurso == "algunos meses"] <- NA
	
	graph13 <- ggplot(datos2, aes(accrecurso)) +
  geom_bar( fill = rgb(0.36, 0.79, 0.95), colour = "black") + xlab("") + ylab("Frecuencia")  
	graph13 + theme_minimal()
	@
	\caption{Acceso al recurso}
	\end{figure}
	Se puede observar que la mayoria de los pobladores no acceden al recurso hídrico en la \comunidad, sin embargo en menor medida si.\\
	\\
	De los propietarios de la qocha se identifico las siguientes respuestas:
	\begin{figure}[H]
	\centering
	<<catorce, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#reemplazando los valores vacios en NA
	datos2$propietario[datos2$propietario == ""] <- NA
	#datos2$propietario[datos2$propietario == "No"] <- "comunal"
	#datos2$propietario[datos2$propietario == "si"] <- "comunal"
	
	datos2 <- datos2[!is.na(datos2$propietario), ]
	ggplot(datos2, aes(propietario)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	@
	\caption{Propietario del recurso}
	\end{figure}
	Se identifica que la mayoria de los pobladores no saben si son propietarios de las qochas en consulta, un porcentaje menor afirman que las Qochas son de propiedad de la comunidad, finalmente reconocen como propietario al estado.\\
	\\
	Del uso que se da se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<quince, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	datos2$uso[datos2$uso == ""] <- NA
	#datos2$uso[datos2$uso == "Estado"] <- NA
	#datos2$uso[datos2$uso == "no sabe"] <- NA
	#datos2$uso[datos2$uso == "privado"] <- NA
	
	datos2 <- datos2[!is.na(datos2$uso), ]
	ggplot(datos2, aes(uso)) +
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))

	
	@
	\caption{Uso del recurso}
	\end{figure}
	De los pobladores que tienen acceso al uso del recursos, se identifico que todos lo utilizan para forraje y una parte utiliza para siembra.\\
	\\
	Se consulto si los beneficiarios estan satisfechos con el servicio de las qochas y se tuvo como resultado lo siguiente:
	\begin{figure}[H]
	\centering
	<<dieciseis, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$satisfecho[datos2$satisfecho == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$satisfecho), ]
	ggplot(datos2, aes(satisfecho)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	@
	\caption{Satisfaccion con el uso del recurso}
	\end{figure}
	Como se puede observar, es mayor la población que no esta satisfecha con el uso del recurso hidríco en su comunidad, asu ves este no cuenta con un nivel adecuado para brindar el servicio. Finalmente hay un grupo pequeño que sí esta satisfecho con el recurso hídrico que recibe.\\
	\\
	Se consulto que otra fuente de agua usa en reemplazo del recurso, de los cual se tiene como resultado:
	\begin{figure}[H]
	\centering
	<<diecisiete, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	#table(datos2$Ocaptacion)
	datos2$Ocaptacion[datos2$Ocaptacion == ""] <- NA
	datos2$Ocaptacion[datos2$Ocaptacion == "Otro manante"] <- "Manantes"
	datos2$Ocaptacion[datos2$Ocaptacion == "manantial"] <- "Manantes"
	datos2$Ocaptacion[datos2$Ocaptacion == "Manante Manante"] <- "Manantes"
	datos2$Ocaptacion[datos2$Ocaptacion == "manantes"] <- "Manantes"
	datos2$Ocaptacion[datos2$Ocaptacion == "Manante"] <- "Manantes"
	#probando eliminando NA
	datos2 <- datos2[!is.na(datos2$Ocaptacion), ]
	ggplot(datos2, aes(Ocaptacion)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	@
	\caption{Otro recurso que suple la oferta del recurso}
	\end{figure}
	Se observa que el recurso que reemplaza es el manantial y riachuelo, siendo el segundo de mayor uso, finalmente se ha encontrado poblacion que no tiene acceso al recurso hídrio\\
	\\
	Se consulto cual es la razon por la que los usuarios no se sienten satisfechos y se identifico lo siguiente:
	\begin{figure}[H]
	\centering
	<<dieciocho, echo = FALSE, fig.width=5, fig.height=5, warning = FALSE, message = FALSE>>=
	#esta informacion debe ser tratada como data minning
	#probando un metodo para la nube de palabras:
	#isntalando el paquete
  setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	#datos <- read.csv("prueba_uno.csv")
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#install.packages("wordcloud2")
	#install.packages("tm")
	#install.packages("SnowballC")
	#install.packages("RcolorBrewer")
	#install.packages("webshot")
	
	library(wordcloud2)
	library(tm)
	library(webshot)
	#webshot::install_phantomjs()
  datos2$Razon[datos2$Razon == "no hay mucha agua"] <- "poca agua"
  datos2$Razon[datos2$Razon == "bajo la cantidad de agua ,"] <- "poca agua"
  datos2$Razon[datos2$Razon == "agua de lluvia, falta agua"] <- "poca agua"
  datos2$Razon[datos2$Razon == "bajo la cantidad de agua"] <- "poca agua"
  datos2$Razon[datos2$Razon == "Hay poca agua"] <- "poca agua"
  datos2$Razon[datos2$Razon == "No hay mucha agua"] <- "poca agua"
	datos2$Razon[datos2$Razon == "No hay mucha agua"] <- "poca agua"
	
  
	noNaa <- datos2$Razon[!is.na(datos2$Razon)]
	frecuencias <- table(noNaa)
	graph18 <- wordcloud2(data = frecuencias, size = 1.5)
	graph18
	
	#table(datos2$Razon)
	@
	\caption{Razon de descontento}
	\end{figure}
	Debido a que las respuestas no son alternativas y son abiertas, se opto por utilizar mineria de datos para estas, de la cual se ha identificado que la opinion con mayor frecuencia es que el descontento se debe a la poca disponibilidad de agua o sequia.
	\section{Aspectos socioeconomicos}
	Se consulto cuantos integrantes de su familia trabajan, de lo cual se tiene la siguiente informacion
	\begin{figure}[H]
	\centering
	<<diecinueve, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$uso[datos2$integrantes == ""] <- NA
	datos2$uso[datos2$integrantes == "No sabe"] <- NA
	#datos2$uso[datos2$integrantes == "No sabe"] <- NA
	
	#probando eliminando NA
	datos2 <- datos2[!is.na(datos2$integrantes), ]
	ggplot(datos2, aes(integrantes)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	#table(datos2$integrantes)
	@
	\caption{cantidad de integrantes que trabajan en la familia}
	\end{figure}
	Se observa que en la mayoria de familias trabajan con dos integrantes, seguido de un solo integrante que labora, sin embargo hay familias que trabajan con 5 integrantes, finalmente hay 3, 4 o 6 integrantes que trabajan en la familia\\
	\\
  Se consulto el ingreso familiar promedio de la familia de lo cual se tiene lo siguiente:
	\begin{figure}[H]
	\centering
	<<veinte, echo = FALSE, fig.width=8, fig.height=5>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$ingresof[datos2$ingresof == ""] <- NA
	datos2$ingresof[datos2$ingresof == "No sabe"] <- NA
	datos2$ingresof <- as.numeric(datos2$ingresof)
	
	datos2 <- datos2[!is.na(datos2$ingresof), ]
	ggplot(datos2, aes(ingresof)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Ingreso promedio de las familias de \comunidad}
	\end{figure}
	Se puede observar que el promedio de ingreso familiar en la comunidad de Chilloroya se sitúa entre 400 a 500 soles, seguido de algunas familias tienen un ingreso que varia de 400 a 950 soles, tambien se pudo obervar que el ingreso propmedio de la familia es de 3000 soles, finalmente se encontró que hay familias donde el ingreso varia entre 1000 a 1500 soles.\\
	\\
	Combinando la cantidad de integrantes que trabajan y el ingreso promedio mensual, se tiene lo siguiente:
	
	\begin{figure}[H]
	\centering
	<<nose, echo = FALSE, fig.width=5, fig.height=7>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$ingresof[datos2$ingresof == ""] <- NA
	datos2$ingresof[datos2$ingresof == "No sabe"] <- NA
	datos2$ingresof <- as.numeric(datos2$ingresof)
	datos2$uso[datos2$integrantes == ""] <- NA
	ggplot(datos2, aes(ingresof, integrantes), na.rm = TRUE) +
	  geom_count(color = rgb(0.36, 0.79, 0.95))
	
	
	
	@
	\caption{Ingreso promedio de las familias de \comunidad en funciona a la cantidad de persionas que trabajan}
	\end{figure}
Los datos del gráfico muestran que 6 integrantes de la familia pueden generar un ingreso promedio mensual de 3000 soles, seguido de 2 integrantes que pueden generar un ingreso que varie de 500 a 1500 soles, tambien se observa que 5 integrantes tienen un promedio mensual que varia entre 600 a 700 soles.
	\section{Aspectos sociales y culturales}
	De la religion que los encuestados creen se tiene la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<veintiuno, echo = FALSE, fig.width=5, fig.height=5>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$religion[datos2$religion == ""] <- NA
	datos2$religion[datos2$religion == "Cristiano"] <- "cristiano"
	
	#table(datos2$religion)
	
	#probando eliminando NA
	datos2 <- datos2[!is.na(datos2$religion), ]
	ggplot(datos2, aes(religion)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	@
	\caption{Religion de los pobladores de \comunidad}
	\end{figure}
	Se puede observar que la mayoria de los encuestados es de religion católica, seguido de la religió Adventista, Israelita y  finalmente existen pobladores que pertenecen a ninguna religión.\\
	\\
	De la pregunta respecto a si alguna organización cultural se dedica a la realizacion de las actividades de las qochas
	\begin{figure}[H]
	\centering
	<<veintidos, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$orcultural[datos2$orcultural == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$orcultural), ]
	ggplot(datos2, aes(orcultural)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	@
	\caption{Organizacion dedicada a actividades culturales en la qocha}
	\end{figure}
	Se observa que la mayoria de los pobladores indica que no existe alguna organizacion dedicada a la realización de actividades culturales en la Qocha, mientras que solo una persona señala que no sabe.  
	\section{Aspectos de gestion}
	De la consulta de la existencia de algun comite encargardo del funcionamiento de la qocha se obtuvo lo siguiente:
	\begin{figure}[H]
	\centering
	<<veintitres, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$uso[datos2$comite == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$comite), ]
	ggplot(datos2, aes(comite)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Comite encargado de la gestion del uso de la qocha en \comunidad}
	\end{figure}
	Se observa que la mayoria de los encuestados indican que no existe un comite de gestión encargado del uso de la qocha, además un pequeño grupo señalan que si hay un comité.\\
	\\
	De la frecuencia al año con que se realizan actividades de mantenimiento o prevención
	\begin{figure}[H]
	\centering
	<<veinticuatro, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$faena[datos2$faena == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$faena), ]
	ggplot(datos2, aes(faena)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Frecuencia de realizacion de actividades de faena en la qocha}
	\end{figure}
	Del gráfico se puede ver la frecuencia de realización de la faena es semestral lo cual quiere decir estás se práctican cada 6 meses; asimismo hay pobladores que llevan las faenas trimestralmente, un grupo reducido indica que se realiza anualmente.\\
	\\
	De la frecuencua, al año, con que se paga al comite se tiene la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<veinticinco, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$pago[datos2$pago == ""] <- NA
	datos2$pago[datos2$pago == "No pagan"] <- "No paga"
	
	
	datos2 <- datos2[!is.na(datos2$pago), ]
	ggplot(datos2, aes(pago)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	@
	\caption{Pago realizado para el comite de gestion de qochas en \comunidad}
	\end{figure}
	El dato del gráfico muestra que los pobladores de la comunidad de Chilloroya no pagan al comité de gestión. lo cual podría indicar que la comunidad tiene entendido sobre el derecho de acceso al agua, además existe un principio de seguridad del mismo donde se promueve y vela por este recurso hídricos\\
	\\
	Existe alguna organizacion externa dedicada al cuidado de la qocha, de la cual se tiene la siguiente respuesta
	\begin{figure}[H]
	\centering
	<<veintiseis, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$externacui[datos2$externacui == ""] <- NA
	
	
	datos2 <- datos2[!is.na(datos2$externacui), ]
	ggplot(datos2, aes(externacui)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	

	@
	\caption{Organizacion externa encargada del cuidado de la qocha}
	\end{figure}
	Se observa que todos los encuestados indican que no existe una organización externa encargada del cuidado de qochas en la \comunidad, solo un pequeño porcentaje de pobladores señalan que cuidan las Qochas. Este último podría indicar que existe una organiación en la protección por parte de los pobladores de la zona\\
	\\
	Se explicó informacion respecto a los beneficios del proyecto, por lo que se consulto si la persona estaria dispuesta en apoyar durante la fase de funcionamiento que comprende la operacion y mantenimiento, de lo cual se tiene la siguiente respuesta:
	\begin{figure}[H]
	\centering
	<<veintisiete, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$apoyo[datos2$apoyo == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$apoyo), ]
	ggplot(datos2, aes(apoyo)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	#para hacer un pie chart, es posible omitiendo los NA
	#graph27 <- ggplot(datos2, aes(apoyo, fill = apoyo), na.rm = TRUE) +
	  #geom_bar() + xlab("Titulo en vertical") + ylab("Titulo en horizontal") +
	  #coord_polar()
	#graph27 + theme_minimal()
	@
	\caption{Disposicion de apoyo durante la ejecucion del proyecto}
	\end{figure}
	Se observa que todos los encuestados estarían dispuestos a apoyar durante la fase de ejecución del proyecto, en específico en la etapa operación y mantenimento de las presas o Qochas asimismo en las actividades que se intervenga con la parte agronómica en la comunidad.\\
	\\
	Explicada la constitución de un comite encargado de la etapa de funcionamiento del proyecto se consulto la predisposicion de los encuestados a pagar una cuata maxima, de la cual se obtuvo las siguientes cantidades:
	\begin{figure}[H]
	\centering
	<<veintiocho, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$cuotaapoyo[datos2$cuotaapoyo == ""] <- NA
	
	#si es posible omitir NA, pero por la uniformidad se utilizara las barras
	#graph28 <- ggplot(datos2, aes(cuotaapoyo, fill = cuotaapoyo), na.rm = TRUE) +
	#  geom_bar() + xlab("Titulo en vertical") + ylab("Titulo en horizontal") +
	#  coord_polar()
	#graph28 + theme_minimal()
	
	datos2 <- datos2[!is.na(datos2$cuotaapoyo), ]
	ggplot(datos2, aes(cuotaapoyo)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	@
	\caption{Disposicion a pagar cuota durante el periodo de funcionamiento del proyecto}
	\end{figure}
	Se puede observar que entre quienes esten dispuestos a pagar y no, la proporción es alejado, siendo mas alta quienes si apoyarian a pagar, esto podría indicar que los pobladores de la comunidad están comprometidos con la labor que van a realizar despues ejecutado el proyecto en la zona, mientras hay encuestados que no están dispuestos a pagar despues ejecutado el proyecto\\
	\\
	De la consulta respecto a la percepción de un adecuado uso de las qochas por parte de las personas, se tiene la siguiente informacion:
	\begin{figure}[H]
	\centering
	<<veintinueve, echo = FALSE, fig.width=5, fig.height=3>>=
	setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	library(stringr)
	library(ggplot2)

	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	#modifica aquellos valores vacios por NA.
	datos2$usoadecuado[datos2$usoadecuado == ""] <- NA
	
	datos2 <- datos2[!is.na(datos2$usoadecuado), ]
	ggplot(datos2, aes(usoadecuado)) + 
	  geom_bar(fill = rgb(0.36, 0.79, 0.95))
	
	
	@
	\caption{Percepcion de uso adecuado de las qochas}
	\end{figure}
	En este caso las dos alternativas tienen la misma proporción, esto concluye que las Qochas son usadas en el riego de sus parcelas sí las condiciones geográficas lo permite, Finalmente existen familias que no usan adecuadamente el agua de la Qocha.\\
	\\
	\section{Aspectos de medio ambiente}
	Perjuicio en la actividad agropecuaria debido al cambio climatico
	\begin{figure}[H]
	\centering
	<<treinta, echo = FALSE, fig.width=4, fig.height=4, warning = FALSE, message = FALSE>>=
  setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	
	
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#datos2 <- subset(datos, Comunidad == "chilloroya")
	
	#install.packages("wordcloud2")
	#install.packages("tm")
	#install.packages("SnowballC")
	#install.packages("RcolorBrewer")
	#install.packages("webshot")
	
	library(wordcloud2)
	library(tm)
	library(webshot)
	#webshot::install_phantomjs(force = TRUE)
  
	#datos2$Climatico[datos2$Climatico == "si"] <- NA
  
	
	noNaa <- datos2$Climatico[!is.na(datos2$Climatico)]
	frecuencias <- table(noNaa)
	graph18 <- wordcloud2(data = frecuencias, size = 3)
	graph18
	@
	\caption{Perjuicio en la actividad agropecuaria debido al cambio climatico}
	\end{figure}
	Como se aprecia los perjuicios producidos por el cambio climático identificados por los encuestados son los siguientes: no hay nevados, poca agua, friaje y menor produccion. esto podría indicar que el caletamiento global es notorio, tambien los actores directos que viven en las zonas altas, propician el derretimiento, alteración de la lluvia con las prácticas que realizan en el pastoreo y agricultura.\\
	\\
	Especies de flora identificadas en la qocha
	
	\begin{figure}[H]
	\centering
	<<treintayuno, echo = FALSE, fig.width=5, fig.height=5, warning = FALSE, message = FALSE>>=
  setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	
	
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	library(wordcloud2)
	library(tm)
	library(webshot)
	#webshot::install_phantomjs(force = TRUE)
  
	datos2$Flora[datos2$Flora == "si"] <- NA
  #datos2$Flora[datos2$Flora == "nevada aumento"] <- NA
  datos2$Flora[datos2$Flora == "ninguno"] <- NA
	datos2$Flora[datos2$Flora == "Ninguno"] <- NA
  
	noNaa <- datos2$Flora[!is.na(datos2$Flora)]
	frecuencias <- table(noNaa)
	graph18 <- wordcloud2(data = frecuencias, size = 1.5)
	graph18
	@
	\caption{Especies de flora identificadas en la qocha}
	\end{figure}
	Se puede observar que las especies de flora identificadas es Paja, Pasto natural, Paja brava  mientras que las demas son Paja, paja brava.\\
	\\
	Especies de fauna identificadas en la qocha
	\begin{figure}[H]
	
	\centering
	<<treintaydos, echo = FALSE, fig.width=5, fig.height=5, warning = FALSE, message = FALSE>>=
  setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	
	
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	library(wordcloud2)
	library(tm)
	library(webshot)
  
	#datos2$Flora[datos2$Flora == "patos"] <- "pato"
  
	noNaa <- datos2$Fauna[!is.na(datos2$Fauna)]
	frecuencias <- table(noNaa)
	graph18 <- wordcloud2(data = frecuencias, size = 1.5)
	graph18
	@
	\caption{Especies de fauna identificadas en la qocha}
	\end{figure}
	Se observa que la especie de fauna identificada con mayor frecuencia es Sapo, esto indica que el medio ambiente donde habitan estas especies de anfibios están preservadas de una manera natural, no hay intervención química que alteren la vida. Finalmente no se han identificado ninguna otra especie en la Qocha.\\
	\\
	Especies vegetales nativas en extincion.
	\begin{figure}[H]
	\centering
	<<treintaytres, echo = FALSE, fig.width=5, fig.height=5, warning = FALSE, message = FALSE>>=
  setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	
	
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	library(wordcloud2)
	library(tm)
	library(webshot)
  
	#datos2$Vegper[datos2$Vegper == "ninguno"] <- NA
	#datos2$Vegper[datos2$Vegper == "pato"] <- NA
	datos2$Vegper[datos2$Vegper == "chalhua"] <- NA
  
	noNaa <- datos2$Vegper[!is.na(datos2$Vegper)]
	frecuencias <- table(noNaa)
	graph18 <- wordcloud2(data = frecuencias, size = 1.5)
	graph18
	@
	\caption{Especies vegetales en extincion}
	\end{figure}
	De las especies vegetales en peligro de extinción, se ha identificado el pasto, Tacalafra y Oqoruro, se podría concluir que el sobrepastoreo en la comunidad es la principal causa, así como tambien la escases del agua en la parte alta dificulta realizar activides de riego para estas especies. Finalmente hay pobladores que afirman que ninguna especie está riesgo de desaparecer del lugar.\\
	\\
	Modificaciones identificadas en la turba de la qocha.
	\begin{figure}[H]
	\centering
	<<treintaycuatro, echo = FALSE, fig.width=5, fig.height=5, warning = FALSE, message = FALSE>>=
  setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	library(wordcloud2)
	library(tm)
	library(webshot)
	datos2$Turba[datos2$Turba == "esta desapareciendo"] <- "Esta desapareciendo"
	datos2$Turba[datos2$Turba == "se esta desapareciendo"] <- "Esta desapareciendo"
	datos2$Turba[datos2$Turba == "Se esta desapareciendo"] <- "Esta desapareciendo"
	datos2$Turba[datos2$Turba == "se han secado"] <- "Se esta secando"
	datos2$Turba[datos2$Turba == "seca"] <- "Se seca"
	datos2$Turba[datos2$Turba == "Seco"] <- "Se seca"
	#datos2$Turba[datos2$Turba == "se esta secando"] <- "se seca"
	
	noNaa <- datos2$Turba[!is.na(datos2$Turba)]
	frecuencias <- table(noNaa)
	graph18 <- wordcloud2(data = frecuencias, size = 1.5)
	graph18
	@
	\caption{Alteraciones de la turba en la qocha}
	\end{figure}
	Se observa que los pobladores identifican que la turba se esta secando y desapareciendo. Lo cual podría afirmar que hay pérdida de los factores que las mantiene como es el caso de las especies que habitan el lugar y el decaimiento de otras especies originarias en el lugar\\
	\\
	Alteraciones en la precipitación pluvial en la zona de la qocha
	\begin{figure}[H]
	\centering
	<<treintaycinco, echo = FALSE, fig.width=5, fig.height=5, warning = FALSE, message = FALSE>>=
  setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	library(wordcloud2)
	library(tm)
	library(webshot)
  
	datos2$Lluvia[datos2$Lluvia == "ninguno"] <- "Ninguno"
	#datos2$Lluvia[datos2$Lluvia == "menos lluvias"] <- "menos lluvia"
  
	noNaa <- datos2$Lluvia[!is.na(datos2$Lluvia)]
	frecuencias <- table(noNaa)
	graph18 <- wordcloud2(data = frecuencias, size = 1.5)
	graph18
	@
	\caption{Alteraciones pluviales en las zonas de qochas}
	\end{figure}
	La mayor alteracion pluvial que se ha indetificado es que hay menos lluvias en la zona de las qochas. esto podría indicar que es la cuasa de el actual fenómeno del Niño, lo cual afecta a la producción y alimentación de la poblacion en la comunidad de Chilloroya.\\
	\\
	Alteraciones en el suelo en la zona de la qocha
		\begin{figure}[H]
	\centering
	<<treintayseis, echo = FALSE, fig.width=5, fig.height=5, warning = FALSE, message = FALSE>>=
  setwd("H:/Gore Cusco/IMA/pi alto apurimac/proyecto/encuesta")
	datos <- read.csv("ask.csv")
	datos$Comunidad <- trimws(datos$Comunidad)
	#para eliminar los espacios en blanco de
	datos <- data.frame(lapply(datos, trimws))
	datos2 <- subset(datos, Comunidad == "chilloroya")
	
	library(wordcloud2)
	library(tm)
	library(webshot)
  
	#datos2$Lluvia[datos2$Lluvia == "mas luvia"] <- "mas lluvia"
	
	#datos2$Lluvia[datos2$Lluvia == "menos lluvias"] <- "menos lluvia"
  
	noNaa <- datos2$Lluvia[!is.na(datos2$Lluvia)]
	frecuencias <- table(noNaa)
	graph18 <- wordcloud2(data = frecuencias, size = 1.5)
	graph18
	
	View(datos2)
	
	#table(datos2$Lluvia)
	
	@
	\caption{Alteraciones en el suelo de las zonas de qochas}
	\end{figure}
	
	\begin{center}
		\textbf{Atentamente}
	\end{center}
	
	
	\begin{figure}[H]
		\centering
		\includegraphics[scale=0.35]{H:/Gore Cusco/img/firma}
	\end{figure}
	\thispagestyle{lastpage}




\end{document}
